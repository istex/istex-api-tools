...
module.exports = function(options, cb) {
  ...
  request('https://api.istex.fr/document/?q=*&output=doi&size=1000&rankBy=random&sid=istex-api-tools', function (error, response, body) {
    ...
    async.mapLimit(body.hits, 50, function (hit, cb) {
      ...
      if (doi) {
        request('https://api.oadoi.org/' + doi + '?email=api-users@listes.istex.fr', function (err, response, oadoiBody) {
